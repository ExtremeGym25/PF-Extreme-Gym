...
private async handleCustomerCreated(event: Stripe.Event) {
  ...
  if (!customer.email) {
    this.logger.warn('❌ No se proporcionó un email válido para el customer.');
    return;
  }
  ...
  if (user) {
    ...
    this.logger.warn(`⚠️ No se encontró un usuario con el email ${customer.email} para asociar el customerId.`);
  }
}
  ...
  private async handleInvoicePaymentSucceeded(event: Stripe.Event) {
    ...
    this.logger.log(`📩 Recibido invoice.payment_succeeded para customerId: ${customerId}`);
    ...
    if (!user) {
      this.logger.warn(`⚠️ No se encontró un usuario con el customerId: ${customerId}`);
      return;
    }
    ...
    const isPremium = invoice.lines.data.some(line =>
      line.pricing?.price_details?.price &&
      [premiumMonthlyPriceId, premiumAnnualPriceId].includes(line.pricing.price_details.price)
    );
...
private async handleSubscriptionUpdated(event: Stripe.Event) {
  ...
  this.logger.log(`📩 Recibido customer.subscription.updated para customerId: ${customerId}, status: ${status}`);
